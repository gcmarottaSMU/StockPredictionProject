...
# Inverse transform predictions requires reconstructing the 2-feature structure
...
predictions_gru_vol = scaler_gru_vol.inverse_transform(predictions_combined)[:, 0] # Extract 'Close' prediction

# Inverse transform actual y_test requires the corresponding scaled 'Volume'
start_index_y_test = train_size_gru_vol + SEQUENCE_LENGTH
end_index_y_test = start_index_y_test + len(y_test_gru_vol)
actual_volume_scaled_test = scaled_data_gru_vol[start_index_y_test:end_index_y_test, 1] # Get Volume column for test period
y_test_combined = np.concatenate((y_test_gru_vol.reshape(-1, 1), actual_volume_scaled_test.reshape(-1, 1)), axis=1)
y_test_gru_vol_unscaled = scaler_gru_vol.inverse_transform(y_test_combined)[:, 0] # Extract actual 'Close'
